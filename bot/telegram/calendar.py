...
def _days(year: int, month: int, prefix: str, minimum: date | None = None):
    rows = [[InlineKeyboardButton(text=name, callback_data="calendar_noop")
             for name in ("Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс")]]
    for week in calendar.Calendar(firstweekday=0).monthdayscalendar(year, month):
        row = []
        for weekday, day in enumerate(week):
            if day == 0:
                row.append(InlineKeyboardButton(text=" ", callback_data="calendar_noop"))
                continue
            ...
            row.append(InlineKeyboardButton(
                ...
                callback_data=f"{prefix}:{value.isoformat()}" if enabled else "calendar_noop",
            ))
        rows.append(row)
...
def _vacation_buttons(vacations, *, prefix: str = "editvac"):
    return _buttons([
        (f"{item.start_date:%d.%m.%Y} - {item.end_date:%d.%m.%Y}", f"{prefix}:{item.id}")
...
def create_calendar_router(service: VacationService, settings: Settings) -> Router:
    ...
    async def start_selection(target_id: int, privileged: bool, state: FSMContext,
                              send, edit_vacation_id: int | None = None) -> None:
        current = date.today().year
        await state.set_state(VacationForm.selecting)
        await state.set_data({"employee_id": target_id, "privileged_mode": privileged,
                              "edit_vacation_id": edit_vacation_id})
        years = range(current - 5, current + 2) if privileged else (current, current + 1)
        await send("Выберите год начала отпуска:",
    ...
    async def my_vacations(message: Message) -> None:
        if message.from_user is None:
            return
        employee = actor(message.from_user.id)
        if employee is None:
        ...
        if not vacations:
            ...
            return
        await message.answer("Выберите отпуск для редактирования:",
    ...
    async def edit_vacation(query: CallbackQuery, state: FSMContext) -> None:
        ...
        if employee is None or not can_manage(employee, service.database.get_employee(vacation.employee_id)):
            await query.answer("Недостаточно прав.", show_alert=True)
            return
        await start_selection(vacation.employee_id, employee.role == "owner" or employee.is_team_lead, state,
                              query.message.edit_text, vacation.id)
        await query.answer()
    ...
    async def request_delete(query: CallbackQuery) -> None:
        ...
        await query.message.edit_text(
            ...
            reply_markup=_buttons([("Подтвердить удаление", f"confirmdeletevac:{vacation.id}")], 1),
        )
        await query.answer()
    ...
    async def confirm_delete(query: CallbackQuery) -> None:
        ...
        await query.message.edit_text("Отпуск удален.")
        await query.answer()
    @router.message(Command("employees"))
    async def employees(message: Message) -> None:
        if message.from_user is None:
            return
        current_actor = actor(message.from_user.id)
        if current_actor is None or current_actor.role != "owner" and not current_actor.is_team_lead:
            await message.answer("Команда доступна владельцу и тимлиду.")
            return
        items = service.database.list_employees()
        if current_actor.is_team_lead and current_actor.role != "owner":
            items = [item for item in items if item.team_lead_id == current_actor.id]
        await message.answer("Доступные сотрудники:", reply_markup=_buttons([
            (item.full_name, f"employee:{item.id}") for item in items
        ], 1))
    ...
    async def employee_actions(query: CallbackQuery) -> None:
        ...
        if can_assign_roles(current_actor):
            items.extend([
                ("Сотрудник", f"employee_role:{employee.id}:employee"),
                ("Включить свойство тимлида", f"employee_lead:{employee.id}:on"),
                ("Отключить свойство тимлида", f"employee_lead:{employee.id}:off"),
                ("Гость", f"employee_role:{employee.id}:guest"),
                ("Владелец", f"employee_role:{employee.id}:owner"),
                ("Назначить тимлида", f"chooselead:{employee.id}"),
                ("Назначить ментора", f"choosementor:{employee.id}"),
            ])
        await query.message.edit_text(
            f"{employee.full_name}\nРоль: {ROLE_LABELS.get(employee.role, employee.role)}",
    ...
    async def change_team_lead_property(query: CallbackQuery) -> None:
        ...
        await query.message.edit_text(f"{employee.full_name}\nСвойство тимлида {status}.")
        await query.answer()
    ...
    async def choose_team_lead(query: CallbackQuery) -> None:
        ...
        leads = [item for item in service.database.list_employees()
                 if item.is_team_lead and item.id != employee_id]
        items = [(item.full_name, f"assignlead:{employee_id}:{item.id}") for item in leads]
        items.append(("Без тимлида", f"assignlead:{employee_id}:none"))
        await query.message.edit_text("Выберите тимлида:", reply_markup=_buttons(items, 1))
        await query.answer()
    ...
    async def choose_mentor(query: CallbackQuery) -> None:
        ...
        if current_actor is None or not can_manage(current_actor, target):
            ...
            return
        candidates = [
            item for item in service.database.list_employees()
            if item.id != employee_id and item.role != "guest"
        ]
        items = [(item.full_name, f"assignmentor:{employee_id}:{item.id}") for item in candidates]
        items.append(("Без ментора", f"assignmentor:{employee_id}:none"))
        await query.message.edit_text("Выберите ментора:", reply_markup=_buttons(items, 1))
        await query.answer()
    ...
    async def employee_vacations(query: CallbackQuery) -> None:
        ...
        if not vacations:
            await query.message.edit_text(f"{employee.full_name}\nОтпусков нет.",
                reply_markup=_buttons([("Добавить отпуск", f"ownervacation:{employee.id}")], 1))
        else:
            await query.message.edit_text(
                f"{employee.full_name}\nВыберите отпуск:",
                reply_markup=_vacation_buttons(vacations),
    ...
    async def year(query: CallbackQuery, state: FSMContext) -> None:
        ...
        today = date.today()
        if not data.get("privileged_mode") and year_value not in {today.year, today.year + 1}:
            await query.answer("Этот год недоступен.", show_alert=True)
            return
        first = today.month if year_value == today.year and not data.get("privileged_mode") else 1
        await query.message.edit_text("Выберите месяц начала:",
                                      reply_markup=_months(year_value, "vsm", first))
        await query.answer()
    ...
    async def start_month(query: CallbackQuery, state: FSMContext) -> None:
        ...
        await query.message.edit_text("Выберите первый день:",
    ...
    async def start_day(query: CallbackQuery, state: FSMContext) -> None:
        ...
        await query.message.edit_text(f"Начало: {start:%d.%m.%Y}\nВыберите месяц окончания:",
    ...
    async def end_month(query: CallbackQuery, state: FSMContext) -> None:
        ...
        await query.message.edit_text("Выберите последний день:",
    ...
    async def finish(query: CallbackQuery, state: FSMContext) -> None:
        ...
        try:
            ...
            return
        await state.clear()
        await query.message.edit_text(f"Отпуск {action}: {start:%d.%m.%Y} - {end:%d.%m.%Y}.")
        if settings.owner_telegram_id and not data.get("privileged_mode"):
            anomaly_text = "\nАномалии: " + "; ".join(anomalies) if anomalies else ""
            await query.bot.send_message(settings.owner_telegram_id,
                f"{employee.full_name}: отпуск {action} {start:%d.%m.%Y} - {end:%d.%m.%Y}." + anomaly_text)
        await query.answer()
